...
module.exports = (upload) => {
    ...
    imageRouter.get('/test/:filename', async (req, res, next) => {
        try {
            const files = await gfs.delete({ filename: req.params.filename });
            res.status(200).json({
                success: true,
            });
    ...
    imageRouter.post('/', upload.single('file'), async (req, res, next) => {
        try {
            ...
            if (imageFile) {
                ...
                console.log(`File with ID ${req.body.caption} is deleted`);
            }
